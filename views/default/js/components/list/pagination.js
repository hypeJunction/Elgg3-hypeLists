...
import $ from 'jquery';
import hypeList from 'js/components/list/list';
	...
	buildPaginationItem: function (type, pageIndex, active, disabled) {
		...
		switch (type) {
			...
			case 'next' :
				...
				break;
			case 'before' :
				text = (pageIndex === 1 || pageIndex === self.options.totalPages) ? echo(self.options.keyTextRemaining) : echo(self.options.keyTextBefore, [self.options.limit]);
				itemText = (active || disabled) ? null : text;
				attr.rel = 'prev';
				attr.class = 'elgg-before';
				break;
			case 'after' :
				text = (pageIndex === 1 || pageIndex === self.options.totalPages) ? echo(self.options.keyTextRemaining) : echo(self.options.keyTextAfter, [self.options.limit]);
				itemText = (active || disabled) ? null : text;
				attr.rel = 'prev';
...
export default hypeListPagination;
